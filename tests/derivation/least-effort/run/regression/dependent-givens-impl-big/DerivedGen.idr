...
namespace VectMaybeAnyType
  ...
  data AtIndex : Fin n -> MaybeAnyType -> VectMaybeAnyType n -> Type where
    ...
    There : {n : _} -> {0 i : Fin n} -> {0 zs : VectMaybeAnyType n} ->
            AtIndex i x zs -> AtIndex (FS i) x (z::zs)

  export
  Show (AtIndex i t vs) where
    show Here = "!"
    show (There x) = "." ++ show x

%language ElabReflection

checkedGen : Fuel -> {n : Nat} -> (v : VectMaybeAnyType n) -> Gen MaybeEmpty (i ** t ** AtIndex i t v)
checkedGen = deriveGen @{MainCoreDerivator @{LeastEffort}}

main : IO ()
main = runGs
  [ G $ \fl => checkedGen fl [Nothing, Just PrimTy, Just RefTy]
  , G $ \fl => checkedGen fl []
  ]
