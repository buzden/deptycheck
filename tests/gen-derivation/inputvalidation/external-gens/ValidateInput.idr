...
genY_repX_autoimpl : Fuel -> (Fuel -> Gen CanBeEmptyStatic X) => (Fuel -> Gen CanBeEmptyStatic X) => (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b
genY_repX_autoimpl = deriveGen

--- Non-gen externals ---

genY_nongen_autoimpl_list : Fuel -> (Fuel -> List Int) => (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b
genY_nongen_autoimpl_list = deriveGen

genY_nongen_autoimpl_pair : Fuel -> (Fuel -> Gen CanBeEmptyStatic X, Fuel -> Gen CanBeEmptyStatic X') => (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b
genY_nongen_autoimpl_pair = deriveGen

genY_nongen_autoimpl_dpair : Fuel -> (Fuel -> (a ** b ** Gen CanBeEmptyStatic $ Y a b)) => (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b
...
genY_require_self_autoimpl : Fuel -> (Fuel -> Gen CanBeEmptyStatic X) => (Fuel -> (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b) => (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b
genY_require_self_autoimpl = deriveGen

--- Auto-implicits are present inside auto-implicits ---

genY_autoimpl_in_autoimpl : Fuel -> (Fuel -> (Fuel -> (a, b : Type) -> Gen CanBeEmptyStatic $ Y a b) => (a : Type) -> Gen CanBeEmptyStatic (b ** Y a b)) => Gen CanBeEmptyStatic (a ** b ** Y a b)
