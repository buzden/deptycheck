...
mapLk : (xs : List (a, b)) -> LazyLst0 (n ** Lookup n xs)
mapLk []            = []
mapLk ((n, ty)::xs) = (n ** Here ty) :: map (\(n ** lk) => (n ** There lk)) (mapLk xs)

lookupGen : (xs : List (a, b)) -> Gen0 (n ** Lookup n xs)
lookupGen xs = elements $ mapLk xs

lookupGen' : DecEq a => (key : a) -> (xs : List (a, b)) -> Gen0 $ Lookup key xs
lookupGen' key xs = do
  elements $ mapMaybe (\(k ** v) => case decEq key k of No _ => Nothing; Yes prf => rewrite prf in Just v) $ mapLk xs
...
exprGen (More f) = oneOf $ alternativesOf nonRec_exprGen ++
                     [ do pure $ U @{!(lookupGen' _ _)} !(exprGen f)
                     , do pure $ B @{!(lookupGen' _ _)} !(exprGen f) !(exprGen f)
                     ]
