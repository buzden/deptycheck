...
(.totalWeight) : OneOfAlternatives em a -> PosNat
(.totalWeight) oo = foldl1 (+) (oo.gens <&> \x => fst x)
...
data Equiv : Gen lem a -> Gen rem a -> Type where
  ...
  EO : lgs `AltsEquiv` rgs => OneOf @{lalemem} @{lalemcd} (MkOneOf lgs) `Equiv` OneOf @{ralemem} @{ralemcd} (MkOneOf rgs)
...
mapOneOf (MkOneOf gs) f = MkOneOf $ mapTaggedLazy f gs
...
trMOneOf (MkOneOf gs) f with (trMTaggedLazy f gs) proof trm
  _ | Nothing = Nothing
  _ | Just gs' = Just $ MkOneOf gs'
...
mkOneOf : alem `NoWeaker` em =>
          ...
          Gen em a
mkOneOf gens = OneOf $ MkOneOf gens
...
{em : _} -> Monad (Gen em) where
  ...
  (OneOf {alem} (MkOneOf gs) >>= nf) {em=MaybeEmpty} = maybe Empty (mkOneOf {alem=MaybeEmptyDeep}) $
...
forgetAlternatives g@(OneOf {}) = case canBeNotImmediatelyEmpty em of
  ...
  where
    ...
    single g = label "forgetAlternatives" $ OneOf $ MkOneOf [(1, g)]
