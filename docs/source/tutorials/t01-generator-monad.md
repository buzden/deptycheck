...
2.  `vec <- vectOf {n} ...`: *Because* `n` has the value `3`, this line creates and runs a generator for the type `Vect 3 Bool`. We have to pass it as a named argument because it is used at the type definition of `vectOf`.
