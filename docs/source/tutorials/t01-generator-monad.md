...
### Run your generator. Open a REPL and load your file. Then, use the `pick` function to get a single value from your generator:

```text
:exec pick genConstantUser
```
...
### Run it multiple times in the REPL:

```text
:exec pick genRandomAgeUser
:exec pick genRandomAgeUser
```
...
```text
:exec pick genRole
:exec pick genUserType
```
    If you run `:exec pick genUserType` many times, you will notice that `"Standard"` appears much more often than `"Premium"`.
